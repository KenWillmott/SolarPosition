...
In case the geographical locations will be assigned dynamically, or radians are preferred as an angular unit, or for any other reason the SolarPosition class is not required, access to the utility function calculateSolarPosition() that calculates solar position is permitted. See the example sketch solarTimeFunctionDemo for details.
